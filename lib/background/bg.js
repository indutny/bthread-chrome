...
function Client(port) {
  ...
  this._tpoolAdd = function tpoolAdd(host) {
    self.postMessage({ type: 'createThread', host: host });
    self.createThread({ host: host });
  };
  this._tpoolShow = function tpoolShow(host) {
    self.postMessage({ type: 'showThread', host: tpool.active });
  };
  this._tpoolRemove = function tpoolRemove(host) {
    self.postMessage({ type: 'removeThread', host: host });
...
Client.prototype.postMessage = function postMessage(msg) {
  if (this.port)
...
Client.prototype.onDisconnect = function onDisconnect() {
  ...
  Object.keys(this.threads).forEach(function(host) {
    this.removeThread({ host: host }, true);
  }, this);
...
Client.prototype.createThread = function createThread(msg) {
  ...
  // Note: tpool emits `create` and we could get there before reaching
  // next statement
  this.threads[host] = true;
...
Client.prototype.removeThread = function removeThread(msg, gentle) {
...
Client.prototype.createWallet = function createWallet(msg) {
  ...
  w.on('balance', function(b) {
    self.postMessage({
      type: 'balance',
      host: host,
      id: id,
      balance: b.toString(10)
    });
  });

  this._addListener(host, this.threads[host], 'tx', function(tx) {
    w.addTX(tx);
  });

  if (msg.opts && msg.opts.wait === false) {
    done();
  } else {
