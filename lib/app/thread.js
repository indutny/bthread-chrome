...
function Thread(manager, host) {
  ...
  this.preview.append(this.postForm.elem);
};
...
Thread.prototype.onCreateDNS = function onCreateDNS(msg) {
  var self = this;
  var opts = { cancel: false, wait: false };
  this.manager.passphrase.ask(msg.host, opts, function onask(err, r) {
    if (err) {
      // Ask passphrase again
      console.error(err);
      self.manager.passphrase.ask(msg.host, opts, onask);
      return;
    }
    ...
    self.cbs[msg.seq] = function(data) {
      ...
      elem.addClass('alert-info')
          ...
          .append($('<button type="button" class="close" ' +
                    ...
                    '&times;</button>'))
          // Text
          .append($('<p/>').text('This domain does not have a DNS record'))
          .append($('<p/>').text('Please set this as a TXT record of ' +
                                 self.host + ' if you want to be an ' +
                                 'owner of the thread'))
          // Record
          .append($('<pre/>').text(data.record))
          // Note
          .append($('<i/>').text('Note: this is a public key of wallet: ' +
                                 r.name));

      elem.alert();

      self.preview.prepend(elem)
    };
  });
};
...
Thread.prototype.onBalance = function onBalance(msg) {
  var b = new bn(msg.balance);

  this.balance[msg.id] = b;
  this.emit('balance', msg.id, b);
};
...
Thread.prototype.addWallet = function addWallet(r) {
  ...
  if (!this.balance[r.id])
    this.balance[r.id] = new bn(0);
  this.emit('wallet', r);
};
...
function Post(thread, post) {
  ...
  // Set content and make links open in new window
  this.elem.find('.thread-post-content')
           .html(marked(content))
           .find('a')
           .attr('target', '_blank')
  ...
  function post() {
    ...
    self.thread.post(wallet,
      ...
      if (err && err.minBalance) {
        ...
        b = b.sub(new bn(self.thread.balance[wallet], 10));

        self.alert(
          'warning',
          'Not enough balance, please transfer ' + utils.toBTC(b) +
              ' BTC to the ' + walletAddr + ' address');
        self.thread.on('balance', function onBalance(id, b) {
          if (id !== wallet)
            return;
          self.thread.removeListener('balance', onBalance);
          post();
        });
        return;
      } else if (err) {
...
function PassphraseForm(manager) {
  ...
  this.fields = {
    ...
    cancel: this.elem.find('button[type=button]'),
    close: this.elem.find('.close'),
  ...
  this.elem.find('form').submit(function(e) {
    ...
    self.fields.cancel.attr('disabled', true);
    self.fields.close.hide();
...
PassphraseForm.prototype.ask = function ask(host, opts, cb) {
  if (typeof opts === 'function') {
    cb = opts;
    opts = null;
  }
  if (this.host !== null) {
    this.queue.push({ host: host, cb: cb, opts: opts });
    return;
  }
  ...
  if (opts && opts.cancel === false) {
    ...
    this.fields.cancel.attr('disabled', true);
    this.fields.close.hide();
  } else {
    this.elem.modal({
      backdrop: 'static',
      keyboard: true,
    ...
    this.fields.close.show();
    this.fields.cancel.attr('disabled', false);
  }
};
...
PassphraseForm.prototype.finish = function finish(msg) {
  ...
  var self = this;
  this.elem.modal('hide').one('hidden.bs.modal', function() {
    self.clear();
...
PassphraseForm.prototype.clear = function clear() {
